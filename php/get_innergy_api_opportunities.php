...
// Execute cURL
$get_opportunities = callAPI('GET', 'https://app.innergy.com/api/opportunities', false);
$opportunities_array = json_decode($get_opportunities, true);

// LOW-TIER EXCEPTION HANDLING
// Check if API data is loaded else kill the process and send back errors
if (!array_key_exists("Items", $opportunities_array))
{
   // Send bad request back to Ajax to trigger error event
   header('HTTP/1.1 500 Innergy data not loaded');
   $err_msg = "";
   foreach ($opportunities_array['Messages'] as $msg)
      { $err_msg .= $msg."\r\n"; }
   echo $err_msg;
   die();
}

unset($opportunities_array);

echo $get_opportunities;
